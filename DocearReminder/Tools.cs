                        ...
                        else
                        {
                            ...
                            if (lastWords.Count >= 0)
                            {
                                ...
                                foreach (var lastWordItem in lastWords)
                                {
                                    ...
                                    XmlAttribute newNoteCREATED1 = x.CreateAttribute("CREATED");
                                    newNoteCREATED1.Value = (Convert.ToInt64((Createtime - TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1))).TotalMilliseconds)).ToString();
                                    XmlAttribute newNoteMODIFIED1 = x.CreateAttribute("MODIFIED");
                                    newNoteMODIFIED1.Value = (Convert.ToInt64((Edittime - TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1))).TotalMilliseconds)).ToString();
                                    newNoteLastWord.Attributes.Append(lastWordItemA);
                                    newNoteLastWord.Attributes.Append(newNoteCREATED1);
