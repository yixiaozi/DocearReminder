...
using yixiaozi.WinForm.Control.DayView;
...
namespace yixiaozi.WinForm.Control.Calendar
{
    public class DayView : System.Windows.Forms.Control
    {
        ...
        private int appointmentGripWidth = 1;
        ...
        public DayView()
        {
            ...
            scrollbar.SmallChange = halfHourHeight/2;
            scrollbar.LargeChange = halfHourHeight;
            scrollbar.Dock = DockStyle.None;//设置没有滚动条
            scrollbar.Visible = allowScroll;
            scrollbar.Scroll += new ScrollEventHandler(scrollbar_Scroll);
            AdjustScrollbar();
            scrollbar.Value = (startHour * 4 * halfHourHeight);//因为这里的原因吗
            scrollbar.Width =0;
            ...
            this.Renderer = new Office11Renderer();//选择默认主题
        }
        ...
        private int halfHourHeight = 20;
        private bool isShowHover = false;
        ...
        private Appointment hoverAppointment;

        [System.ComponentModel.Browsable(false)]
        public Appointment HoverAppointment
        {
            get { return hoverAppointment; }
            set { hoverAppointment = value; }
        }
        ...
        private int workingMinuteStart = 15;
        ...
        private int workingMinuteEnd = 15;
        ...
        {
            scrollbar.Maximum = (2 * halfHourHeight * 24) + this.HeaderHeight - this.Height;
            scrollbar.Minimum = 0;
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            Appointment appointment = GetAppointmentAt(e.X, e.Y);
            hoverAppointment = appointment;
            if (activeTool != null)
            {
                activeTool.MouseMove(e);
            }
            //Invalidate();
            RaiseAppointmentMouseMove(e);
            //base.OnMouseMove(e);
        }
        ...
        protected override void OnMouseHover(EventArgs e)
        {
            ...
            if ((hoverAppointment != null))
                RaiseAppointmentMouseHover();
            base.OnMouseHover(e);
        }
        protected override void OnMouseLeave(EventArgs e)
        {
            if ((hoverAppointment != null))
                RaiseAppointmentMouseLeave();
            base.OnMouseLeave(e);
        }
        ...
        internal void RaiseAppointmentMouseHover()
        {
            AppointmentMouseHoverEventArgs args = new AppointmentMouseHoverEventArgs(hoverAppointment.Title, hoverAppointment.StartDate, hoverAppointment.EndDate);

            if (AppointmentMouseHover != null)
            {
                AppointmentMouseHover(this, args);
            }
            //selectedAppointment = null;
            //selectedAppointmentIsNew = false;
        ...
        internal void RaiseAppointmentMouseLeave()
        {
            AppointmentMouseLeaveEventArgs args = new AppointmentMouseLeaveEventArgs(hoverAppointment.Title, hoverAppointment.StartDate, hoverAppointment.EndDate);

            if (AppointmentMouseLeave != null)
            {
                AppointmentMouseLeave(this, args);
            }
            //selectedAppointment = null;
            //selectedAppointmentIsNew = false;
        ...
        internal void RaiseAppointmentMouseMove(MouseEventArgs e)
        {
            AppointmentMouseMove?.Invoke(this, hoverAppointment, e);
            //Invalidate();
        }
        #endregion
        ...
        public void StartEditing()
        {
            if (!selectedAppointment.Locked && appointmentViews.ContainsKey(selectedAppointment))
            {
                ...
                editbox.BackColor = selectedAppointment.BorderColor;
                //todo: font
                editbox.Focus();
            }
        }
        ...
        public DateTime GetTimeAt(int x, int y)
        {
            ...
            int hour = (y - this.HeaderHeight + scrollbar.Value) / halfHourHeight/2;//小时
            int minutes = Convert.ToInt16((Double)((y - this.HeaderHeight + scrollbar.Value) % (halfHourHeight*2)) * 60 / (halfHourHeight*2));
            x -= hourLabelWidth;
        ...
        private void DrawHourLabels(PaintEventArgs e, Rectangle rect)
        {
            ...
            for (int m_Hour = 0; m_Hour < 24; m_Hour++)//ok
            {
                ...
                hourRectangle.Height = this.HeaderHeight*2;
        ...
        private void DrawDayHeaders(PaintEventArgs e, Rectangle rect)
        {
            ...
            for (int day = 0; day < daysToShow; day++)//ok
            {
        ...
        private void DrawDay(PaintEventArgs e, Rectangle rect, DateTime time)
        {
            ...
            {
                int y = rect.Top + (hour * halfHourHeight/2) - scrollbar.Value;//获取顶部

                using (Pen pen = new Pen((((hour % 4) == 0) ? 
                    (hour==48|| hour == 32|| hour == 68 || hour == 16 || hour == 76 || hour == 88?Color.DeepSkyBlue : renderer.HourSeperatorColor) : (hour == 31? Color.Pink : renderer.HalfHourSeperatorColor))))//两种颜色
            ...
            try
            {
                int y = rect.Top + (DateTime.Now.Hour * halfHourHeight * 2) + Convert.ToInt32((Convert.ToDouble(DateTime.Now.Minute) / 60) * halfHourHeight * 2) - scrollbar.Value;//获取顶部
                using (Pen pen = new Pen(Color.Red))//两种颜色
                    e.Graphics.DrawLine(pen, rect.Left, y, rect.Right, y);
            }
            catch (Exception)
            {
            ...
            renderer.DrawDayGripper(e.Graphics, rect, appointmentGripWidth);//画间隔
            ...
            {
                Rectangle selectionRectangle = GetHourRangeRectangle(selectionStart, selectionEnd, rect);
                int minutes = Convert.ToInt32((selectionEnd - selectionStart).TotalMinutes);

                renderer.DrawHourRange(e.Graphics, selectionRectangle, false, true, minutes.ToString());
            }
        }
        ...
        private HalfHourLayout[] GetMaxParalelAppointments(List<Appointment> appointments)
        {
            ...
            foreach (Appointment appointment in appointments)
            {
                int firstHalfHour = appointment.StartDate.Hour * 60 + appointment.StartDate.Minute;
                int lastHalfHour = appointment.EndDate.Hour * 60 + appointment.EndDate.Minute;
                ...
                for (int halfHour = firstHalfHour; halfHour < lastHalfHour; halfHour++)
                {
                    ...
                    if (layout == null)
                    {
                        ...
                        layout.Appointments = new Appointment[10000];
        ...
        public event AppointmentMouseHoverEventHandler AppointmentMouseHover;
        public event AppointmentMouseLeaveEventHandler AppointmentMouseLeave;
        public event AppointmentMouseMoveEventHandler AppointmentMouseMove;
