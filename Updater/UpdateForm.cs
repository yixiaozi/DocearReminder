		...
		protected override void OnShown(EventArgs e)
		{
			...
			using (var stm = wrs.GetResponseStream())
			{
				...
				foreach(var entry in zip.Entries)
				{
					...
					try
					{
                        if (entry.FullName.IndexOf(".") >= 0 && ((entry.FullName.EndsWith("exe") || entry.FullName.EndsWith("dll") || entry.FullName.EndsWith("pdb"))))
                        {
					...
					using (var stm1 = entry.Open())
                    {
                        try
                        {
                            if (string.IsNullOrEmpty(d)&& (entry.FullName.IndexOf(".")>=0&& ((entry.FullName.EndsWith("exe")|| entry.FullName.EndsWith("dll")|| entry.FullName.EndsWith("pdb"))||!System.IO.File.Exists(entry.FullName))))
                            {
